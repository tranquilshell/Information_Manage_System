...
public class ManageSelectedCourseFrm extends JInternalFrame {
	...
	protected void deleteSelectedCourse(ActionEvent ae) {
		...
		if(scObj.delete(selected_id)){
			if(courseObj.updateSelectedNum(getCourseIdByName(courseName), -1)){
	...
	protected void editSelectCourseAct(ActionEvent ae) {
		...
		CourseObj courseObj = new CourseObj();
		if(!courseObj.selectedEnable(course.getId())){
		...
		SelectedCourseObj scObj = new SelectedCourseObj();
		if(scObj.isSelected(sc)){
			JOptionPane.showMessageDialog(this, "已经选过这门课程了，不能再选!");
			return;
		}
		if(scObj.updateSelectedCourse(sc)){
			if(courseObj.updateSelectedNum(sc.getCourse_id(),1)){
				if(courseObj.updateSelectedNum(getCourseIdByName(courseName), -1)){
	...
	protected void confirmSelectedCourse(ActionEvent ae) {
		...
		CourseObj courseObj = new CourseObj();
		if(!courseObj.selectedEnable(scourse.getId())){
			JOptionPane.showMessageDialog(this, "该课程已经选满，不能再选!");
			return;
		}
		SelectedCourseObj scObj = new SelectedCourseObj();
	...
	private void getSelectedCourse(SelectedCourse selectedCourse){
		SelectedCourseObj selectedCourseObj = new SelectedCourseObj();
		List<SelectedCourse> selectedCourseList = selectedCourseObj.getSelectedCourseList(selectedCourse);
