...
public class ManageStudentFrm extends JInternalFrame {
	...
	protected void deleteStudent(ActionEvent ae) {
		...
		StudentObj studentObj = new StudentObj();
		if(studentObj.delete(Integer.parseInt(studentListTable.getValueAt(row, 0).toString()))){
	...
	private void setTable(Student student){
		...
		dft.setRowCount(0);
		StudentObj studentObj = new StudentObj();
		List<Student> studentList = studentObj.getStudentList(student);
	...
	private void setStudentClassInfo(){
		...
		studentClassList = classObj.getClassList(new StudentClass());
